...
class ASAPNetsGenerator(BaseNetwork):
    ...
    def __init__(self, opt, hr_stream=None, lr_stream=None, in_nc=3, out_nc=3, fast=False):
        super(ASAPNetsGenerator, self).__init__()
        if lr_stream is None or hr_stream is None:
            lr_stream = dict()
            hr_stream = dict()
        self.num_inputs = in_nc
        ...
        self.highres_stream = ASAPNetsHRStream(self.downsampling, num_inputs=self.num_inputs,
                                               num_outputs=self.output_nc, width=opt.hr_width,
...
class ASAPNetsHRStream(th.nn.Module):
    ...
    def forward(self, highres, lr_params):
        # try:
        assert lr_params.shape[1] == self.num_params, f"incorrect input params, {lr_params.shape}, {self.num_params}"
#         except:
#             lr_params = lr_params.unsqueeze(0)
#             highres = highres.unsqueeze(0)
        ...
        if not(self.coordinates is None):
            ...
            try:
                ...
                highres = th.cat([highres, self.xy_coords[:1]], 1)
